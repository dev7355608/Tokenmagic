...
vec4 morph(vec2 uv, vec2 uvt) {
    ...
    float w0 = progress;
    float w1 = 1.0-w0;
    vec2 sourceMappedCoord = (vFilterCoord+(oc*0.4)*w0) * (outputFrame.zw * inputSize.zw);
...
vec4 waterdrop(vec2 uv, vec2 uvt) {
    ...
    if (distt > progress) {
        ...
        vec2 fuv = (vFilterCoord + (shiftuvt*(1.-progress))) * (outputFrame.zw * inputSize.zw);
...
vec4 swirl(vec2 uv, vec2 uvt) {
    ...
    vec2 sourceMappedCoord = suvfrom * (outputFrame.zw * inputSize.zw);
